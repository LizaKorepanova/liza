Surfaces.prototype.ellipsoid = (x0 = 0, y0 = 0, z0 = 0, a = 10, b = 4, countRing = 5, countPoints = 5) => 
{
    const points = [];
    if (countRing % 2 != 0) 
    { // Если кол-во колец нечетное 
        for (let i = 0; i < Math.PI * 2; i += Math.PI * 2 / countRing) 
        { // Вертикаль
            const y = y0 + b * Math.sin(i);
            let counter = 0;
            let j = 0;
            while (counter != countPoints) 
            {
                const x = x0 + a * Math.cos(i) * Math.cos(j);
                const z = z0 + a * Math.cos(i) * Math.sin(j);
        ...
        { // Вертикаль
            ...
            while (counter != countPoints) 
            {
                const x = a * Math.cos(i) * Math.cos(j);
                const z = a * Math.cos(i) * Math.sin(j);
                points.push(new Point(x, y, z));
                j += Math.PI * 2 / countPoints;
                counter++;
            }
    ...
    {
        edges.push(new Edge(i * countPoints, (i + 1) * countPoints - 1));
        for (let j = 0; j < countPoints - 1; j++) 
        {
            edges.push(new Edge(i * countPoints + j, i * countPoints + j + 1));
        }
    }
    // * Вертикальные линии *
    for (let i = 0; i < countRing; i++) 
